...
const WordTestPage = () => {
    ...
    const handleLogout = () => {
        ...
        if (storedUser?.username) {
            localStorage.removeItem(getUserKey(storedUser.username));
    ...
    useEffect(() => {
        axios.get("/api/main", { withCredentials: true })
            ...
            .catch(() => alert("로그인 상태 확인 실패"));
    }, []);

    useEffect(() => {
        if (!currentUser?.username) return;
        const saved = JSON.parse(localStorage.getItem(getUserKey(currentUser.username)) || "[]");
        setCompletedDates(saved);
    }, [currentUser?.username]);

    function getUniqueRandomSubset(array, count) {
        const uniqueMap = new Map();
        array.forEach((item) => uniqueMap.set(item.spelling, item));
    ...
    useEffect(() => {
        ...
        axios.get("/api/learn/today", { withCredentials: true })
            .then(res => {
                const testList = getUniqueRandomSubset(res.data, 20);
                setWords(testList);
            })
    ...
    useEffect(() => {
        if (isTestFinished && currentUser?.username) {
    ...
    }, [isTestFinished, currentUser?.username]);
